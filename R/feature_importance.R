...
feature_importance <- function(np_data.sub,
  ...
  if(is.list(np_data.knockoff)){
    ...
    colnames(W_imp) <- colnames(np_data.knockoff[[1]])
    ...
    if(test.use != "LCD"){
      ...
      cv_lasso_fit_coef <- cv_lasso_fit_coef[,colnames(cv_lasso_fit_coef) != "(Intercept)", drop = FALSE]

      cv_lasso_fit_coef_orig <- cv_lasso_fit_coef[,match(colnames(np_data.matrix), colnames(cv_lasso_fit_coef))]
      # cv_lasso_fit_coef_orig2 <- cv_lasso_fit_coef[,colnames(cv_lasso_fit_coef) %in% colnames(np_data.matrix)]
      ...
      for (m_ko in 1:m_kos)
      {
        ...
        # cv_lasso_fit_coef_mth_ko2 <- cv_lasso_fit_coef[,colnames(cv_lasso_fit_coef) %in% colnames_mth_ko]
        # max(abs(cv_lasso_fit_coef_mth_ko - cv_lasso_fit_coef_mth_ko2))
    ...
    colnames(W_imp) <- colnames(np_data.knockoff)
    ...
    if (test.use != "LCD"){
      ...
      cv_lasso_fit_coef_orig <- cv_lasso_fit_coef[,match(colnames(np_data.matrix_scale), colnames(cv_lasso_fit_coef))]
      # cv_lasso_fit_coef_orig2 <- cv_lasso_fit_coef[,colnames(cv_lasso_fit_coef) %in% colnames(np_data.matrix_scale)]
      # max(abs(cv_lasso_fit_coef_orig - cv_lasso_fit_coef_orig2))

      cv_lasso_fit_coef_ko <- cv_lasso_fit_coef[,match(colnames(np_data.knockoff.scale), colnames(cv_lasso_fit_coef))]
      # cv_lasso_fit_coef_ko2 <- cv_lasso_fit_coef[,colnames(cv_lasso_fit_coef) %in% colnames(np_data.knockoff.scale)]
