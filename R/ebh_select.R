...
#' @param qkn A numeric scalar in (0, \code{q}] specifying the FDR level used
...
#' @family select
#'
#' @details Select variables with FDR control using the eBH procedure. The
...
#'
#' @export
ebh_select <- function(W_imp, m = 0, q = 0.1, qkn = NULL){
  ...
  # qkn (allow NULL -> set later)
  if (!is.null(qkn)) {
    if (!is.numeric(qkn) || length(qkn) != 1 || is.na(qkn) || qkn <= 0 || qkn > q) {
      stop("`qkn` must be NULL or a numeric scalar in (0, q].", call. = FALSE)
    }
  }
  ...
  # If qkn is NULL, set default now (after validating q)
  if (is.null(qkn)) {
    qkn <- q / 2
  }
  ...
  for(w_it in 1:m){
    ...
    tau <- knockoff::knockoff.threshold(E[w_it,],fdr = qkn)
    E[w_it,] <- (E[w_it,] >= tau) / (1 + sum(E[w_it,] <= -tau))
  }

  E <- (dim(W_imp)[2]) * colMeans(E)

  selected <- ebh_helper(E,q)$rej
